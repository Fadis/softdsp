#ifndef SOFTDSP_SHIFT_LEFT_HPP
#define SOFTDSP_SHIFT_LEFT_HPP
...
namespace softdsp {
  ...
  namespace keywords {
    ...
    class shift_left {
    public:
      shift_left( const Context &context_ ) : tools( context_.get_toolbox() ) {}
      shift_left( const typename Context::toolbox_type &tools_ ) : tools( tools_ ) {}
      ...
      operator()(
        ...
        return return_value<
          ...
          >::type
        >(
          tools->ir_builder.CreateShl( left.value, right.value )
        );
      }
      template< typename LeftType, typename RightType >
      auto operator()(
