#ifndef SOFTDSP_GREATER_HPP
#define SOFTDSP_GREATER_HPP
...
namespace softdsp {
  ...
  namespace keywords {
    ...
    class greater {
    public:
      greater( const Context &context_ ) : tools( context_.get_toolbox() ) {}
      greater( const typename Context::toolbox_type &tools_ ) : tools( tools_ ) {}
      ...
      return_value< bool > operator()(
        ...
        return return_value< bool >(
          tools->ir_builder.CreateFCmpUGT( left.value, right.value )
        );
      }
      template< typename LeftType, typename RightType >
      return_value< bool > operator()(
        ...
        const auto right = cast( tools->as_llvm_value( tools->load( right_ ) ) );
        return return_value< bool >(
          tools->ir_builder.CreateICmpSGT( left.value, right.value )
        );
      }
      template< typename LeftType, typename RightType >
      return_value< bool > operator()(
        ...
        const auto right = cast( tools->as_llvm_value( tools->load( right_ ) ) );
        return return_value< bool >(
          tools->ir_builder.CreateICmpUGT( left.value, right.value )
        );
      }
      template< typename LeftType, typename RightType >
      auto operator()(
