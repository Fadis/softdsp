#ifndef SOFTDSP_DEREFERENCE_HPP
#define SOFTDSP_DEREFERENCE_HPP
...
namespace softdsp {
  ...
  namespace keywords {
    ...
    class dereference {
    public:
      dereference( const Context &context_ ) : tools( context_.get_toolbox() ) {}
      dereference( const typename Context::toolbox_type &tools_ ) : tools( tools_ ) {}
      ...
      operator()(
        ...
        typename boost::enable_if<
          boost::mpl::and_<
            at_least_one_operand_is_llvm_value< ValueType >,
            boost::is_pointer< typename boost::remove_reference< typename get_return_type< ValueType >::type >::type >
          >
        >::type* = 0
      ) {
        const auto value = tools->as_llvm_value( tools->load( value_ ) );
